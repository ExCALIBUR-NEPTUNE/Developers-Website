...
upper and lower case plus underscore '\_`. How best to use underscore has
been carefully considered as it is also needed 
to enable use of 'pothole' convention. There is
...
Given a rule that all mathematical symbols occupy at least two
letters, one use for underscore is as padding for lower-case letters
used as mathematical symbols.  The underscore may precede or follow
the letter, and there is the additional rule that an underscore at the
end of a transliterated variable may be dropped. Another short-cut which
should be helpful is that when two~two-letter symbols are adjacent,
the second will be presumed to be a suffix of the first. 
...
\begin{itemize}
\item `a\_' will represent `a' (as will '\_a')
...
A single letter by itself will represent a single character symbol,thus:

\begin{itemize}
\item `a' will denote $a$
\end{itemize}
