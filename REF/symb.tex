...
then the dimensions depend on context. To emphasise that a quantity is scaled/dimensionless,
place a tilde `\~' above the main part of the name, to express $X$ as a dimensioned quantity, enclose
in parenthesis with optionally units employed, thus $(X \mbox{ in SI})$ or simply $X$
...
$D_\alpha$ & (anomalous) diffusion coefficient for particles of species~$\alpha$  &  $m^2 s^{-1}$ \\
$D_{\kappa\alpha}$ & (anomalous) diffusion coefficient for energy of species~$\alpha$  &  $m^2 s^{-1}$ \\
...
$e_s=w_{ref}|e|$ & unit of electric charge  & $C$ \\
$\emptyset$ &  (Sets) Empty set & \\
$\epsilon_0$ & permittivity of free space  &  $F m^{-1}$, $C^2 s^2 m^{-1}s^{-3}$ \\
...
$j_s=\frac{N_{ref} |e|}{t_s L_s^2}$ & characteristic current density  & $A m^{-2}$ \\
...
$L_n$ & typical lengthscale of density variation & $m$ \\
...
$\mathcal{P}_{\alpha}$ &  Pressure tensor & $N m^{-2}$ \\
...
$\mu_0$ & permeability of free space & $H m^{-1}$, $kg m C^{-2}$ \\
...
$\nu_h$ & plasma kinematic hyperviscosity & $m^4 s^{-1}$ \\
...
$\Phi_s=\frac{m_s L_s^2}{e_st_s^2}$ & scaling for electr(ostat)ic potential  & $V$ \\
...
$R_u$ & Electron heating term (Braginskii) & $W m^{-3}$\\
...
$s_\alpha$ & entropy of species $\alpha$   & ? \\
